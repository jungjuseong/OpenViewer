...
import org.jpedal.examples.viewer.gui.javafx.JavaFXScrollListener;
 ...
 */
public class BrainGUI extends GUI implements GUIFactory {

	private RefreshLayout viewListener;
	protected final BrainButtons fxButtons;
	...
	/**
	 * Multibox for Layout Component to contain memory, cursor and loading bars
	 * IMPORTANT : the width of the multibox needs to be the same size as the
	 * width of pagesToolBar to ensure that it is centered on the page.
	 */
	private HBox multiboxfx;
	private FXProgressBarWithText memoryBarFX;

	// visual display of current cursor coordinates on page
	...
	public BrainGUI(final Stage stage, final PdfDecoderInt decode_pdf, final Values commonValues,
			final GUIThumbnailPanel thumbnails, final PropertiesFile properties) {
	...
	/*
	 * For the following methods getThumbnailScrollBar,
	 * setThumbnailScrollBarVisibility and setThumbnailScrollBarValue: These
	 * methods have not been implemented yet as they do not have the equivalent
	 * to SwingGUI.thumbscroll implemented in FX.
	 */
	@Override
	...
	public void setThumbnailScrollBarVisibility(final boolean isVisible) {
		...
		// making invisible still shows the gap so padding negative to hide it
		if (isVisible)
	...
	private void setLookAndFeel() {
		/**
		 * JavaFX has not look and feel option, 2.2 has a theme called Caspian (the default theme) only. 
		 * FX8 has Moderna and Caspian, which can be set in the class which overrides Application with
		...
		root.getStylesheets().clear();
		root.getStylesheets()
				.add(getClass().getResource("/org/jpedal/examples/viewer/res/css/JavaFXForms.css").toExternalForm());
		root.getStylesheets()
				.add(getClass().getResource("/org/jpedal/examples/viewer/res/css/JavaFXPages.css").toExternalForm());
	}
	...
	private void setupDisplay() {
		...
		menuItems = new BrainMenuItems(properties);
			...
			if (!commonValues.isPDF()) {
				...
				if (!decode_pdf.hasOutline()) {
					...
					if (DecoderOptions.isRunningOnMac) {
						...
						if (outlineTab == -1) // @Simon Change to add JavaFXOutline tab						
	...
	public Object getVerticalScrollBar() {
		// Altered function due to above comments, return thumbscroll only when
		// shown
	...
	/*
	 * Allows user to embed viewer into own application횄占�
	 */
	...
	public void setRootContainer(final Object rawValue) {
		...
		// Used to prevent infinite scroll issue as a preferred size has been
		// set
	...
	public void resetRotationBox() {
		...
		// >>> DON'T UNCOMMENT THIS LINE, causes major rotation issues, only
		// useful for debuging <<<
	...
	private void searchInMenu(final GUISearchWindow searchFrame) {
		...
		fxButtons.addButton(GUIFactory.BUTTONBAR, Messages.getMessage("PdfViewerSearch.Previous"),
				"search_previous.gif", Commands.PREVIOUSRESULT, menuItems, this, currentCommandListener, pagesToolBar,
				navToolBar);
		fxButtons.addButton(GUIFactory.BUTTONBAR, Messages.getMessage("PdfViewerSearch.Next"), "search_next.gif",
				Commands.NEXTRESULT, menuItems, this, currentCommandListener, pagesToolBar, navToolBar);
	 ...
	 * 
	 * @see
	 * org.jpedal.examples.viewer.gui.swing.GUIFactory#init(java.lang.String[],
	 * org.jpedal.examples.viewer.Commands,
	...
	public void init(final Commands currentCommands, final Object currentPrinter) {
		...
		mouseHandler = new BrainMouseListener((PdfDecoderFX) decode_pdf, this, commonValues, currentCommands);
		 ...
		 */
		// menuItems.createMainMenu(true, currentCommandListener, isSingle,
		// commonValues, currentCommands, fxButtons);
		// createSwingMenu(true);
		/**
		 * sets up all the toolbar items
		 */
		fxButtons.addButton(GUIFactory.BUTTONBAR, Messages.getMessage("PdfViewerToolbarTooltip.openFile"), "open.gif",
				Commands.OPENFILE, menuItems, this, currentCommandListener, pagesToolBar, navToolBar);

		fxButtons.addButton(GUIFactory.BUTTONBAR, Messages.getMessage("PdfViewerToolbarTooltip.print"), "print.gif",
				Commands.PRINT, menuItems, this, currentCommandListener, pagesToolBar, navToolBar);

		if (searchFrame != null && (searchFrame.getViewStyle() == GUISearchWindow.SEARCH_EXTERNAL_WINDOW
				|| (searchFrame.getViewStyle() == GUISearchWindow.SEARCH_MENU_BAR && false))) {
			searchFrame.setViewStyle(GUISearchWindow.SEARCH_EXTERNAL_WINDOW);
			fxButtons.addButton(GUIFactory.BUTTONBAR, Messages.getMessage("PdfViewerToolbarTooltip.search"), "find.gif",
					Commands.FIND, menuItems, this, currentCommandListener, pagesToolBar, navToolBar);
		}

		fxButtons.addButton(GUIFactory.BUTTONBAR, Messages.getMessage("PdfViewerToolbarTooltip.properties"),
				"properties.gif", Commands.DOCINFO, menuItems, this, currentCommandListener, pagesToolBar, navToolBar);

		if (commonValues.getModeOfOperation() == Values.RUNNING_PLUGIN) {
			fxButtons.addButton(GUIFactory.BUTTONBAR, Messages.getMessage("PdfViewerToolbarTooltip.about"), "about.gif",
					Commands.ABOUT, menuItems, this, currentCommandListener, pagesToolBar, navToolBar);
		}

		/**
		 * snapshot screen function
		 */
		fxButtons.addButton(GUIFactory.BUTTONBAR, Messages.getMessage("PdfViewerToolbarTooltip.snapshot"),
				"snapshot.gif", Commands.SNAPSHOT, menuItems, this, currentCommandListener, pagesToolBar, navToolBar);
		 ...
		 */
		addCombo(Messages.getMessage("PdfViewerToolbarScaling.text"),
				Messages.getMessage("PdfViewerToolbarTooltip.zoomin"), Commands.SCALING);

		addCombo(Messages.getMessage("PdfViewerToolbarRotation.text"),
				Messages.getMessage("PdfViewerToolbarTooltip.rotation"), Commands.ROTATION);

		fxButtons.addButton(GUIFactory.BUTTONBAR, Messages.getMessage("PdfViewerToolbarTooltip.mouseMode"),
				"mouse_select.png", Commands.MOUSEMODE, menuItems, this, currentCommandListener, pagesToolBar,
				navToolBar);

		fxButtons.getTopButtons().getItems().add(sep);

		if (includeExtraMenus && commonValues.getModeOfOperation() != Values.RUNNING_PLUGIN) {
			fxButtons.addButton(GUIFactory.BUTTONBAR, Messages.getMessage("PdfViewerToolbarTooltip.help"), "help.png",
					Commands.HELP, menuItems, this, currentCommandListener, pagesToolBar, navToolBar);
		}

		// addButton(GUIFactory.BUTTONBAR,Messages.getMessage("PdfViewerToolbarTooltip.buy"),iconLocation+"buy.png",Commands.BUY);
		if (includeExtraMenus && commonValues.getModeOfOperation() != Values.RUNNING_PLUGIN) {
			fxButtons.addButton(GUIFactory.BUTTONBAR, "RSS", "rss.gif", Commands.RSS, menuItems, this,
					currentCommandListener, pagesToolBar, navToolBar);
		}
		...
		if (GUI.debugFX) {
			System.out.println(
					"Swing specific invalidate(), validate(), repaint() not implemented for JavaFX in JavaFxGUI");
		}
		// frame.invalidate();
		// frame.validate();
		// frame.repaint();
		...
		if (commonValues.getModeOfOperation() != Values.RUNNING_APPLET) {
			...
			// Used to prevent infinite scroll issue as a preferred size has
			// been set
			// Used to prevent infinite scroll issue as a preferred size has
			// been set
			final Rectangle2D d = Screen.getPrimary().getVisualBounds();
		...
		final int ALL = -10;
		((BrainMenuItems) menuItems).disableUnimplementedItems(ALL, false);
		fxButtons.disableUnimplementedItems(ALL);
	}

	private void nameElements() {

		((BrainMenuItems) menuItems).getCurrentMenuFX().setId("menuBar");
	...
	protected void createOtherToolBars() {
		...
		topPane.getChildren().add(((BrainMenuItems) menuItems).getCurrentMenuFX());
	...
	private void initMultiBox() {
		...
		// Temp code to demonstrate functionality - move to JavaFXMouseListener
		// when implmented
		// Show coords
		// ((PdfDecoderFX)decode_pdf).setOnMouseEntered(new
		...
		// });
		// Show memory bar
		// ((PdfDecoderFX)decode_pdf).setOnMouseExited(new
		// EventHandler<javafx.scene.input.MouseEvent>(){
		// @Override
		// public void handle(javafx.scene.input.MouseEvent t) {
		...
		// }
		// });
		// ((PdfDecoderFX)decode_pdf).setOnMouseMoved(new
		// EventHandler<javafx.scene.input.MouseEvent>() {
		// @Override
		// public void handle(javafx.scene.input.MouseEvent t) {
		// double maxX =
		// (((PdfDecoderFX)decode_pdf).getBoundsInLocal().getMaxX()) +
		// (dropshadowDepth /2);
		// double maxY =
		// (((PdfDecoderFX)decode_pdf).getBoundsInLocal().getMaxY()) +
		// (dropshadowDepth /2);
		// double minX = ((PdfDecoderFX)decode_pdf).getBoundsInLocal().getMinX()
		// + (dropshadowDepth /2);
		// double minY = ((PdfDecoderFX)decode_pdf).getBoundsInLocal().getMinY()
		// + (dropshadowDepth /2);
		//
		// if(!cursorOverPage){
		...
		// }
		// }
		// //(String.format("X: %d Y: %d", (int)(t.getX() - minX),
		// (int)(t.getY() - minY)));
		// }
		// });
	...
	public void setMultibox(final int[] flags) {
		...
		// LOAD_PROGRESS:
		// if (statusBar.isEnabled() && statusBar.isVisible() &&
		// !statusBar.isDone()) {
		// multibox.removeAll();
		...
		// DOWNLOAD_PROGRESS:
		if (!downloadBar.isDisable() && downloadBar.isVisible() && !downloadBar.isDone()
				&& (decode_pdf.isLoadingLinearizedPDF() || !decode_pdf.isOpen())) {
			multiboxfx.getChildren().clear();
	...
	private boolean setDisplayView2(final int displayView, final int orientation) {
		...
		default:
			...
			if (needsReset) {
				pages = new MultiDisplayFX(this, pageNumber, decode_pdf.getDynamicRenderer(), displayView, comp,
						options, fileAccess);
				// displayRotation = displayRotation;
				pages.setPageRotation(decode_pdf.getDisplayRotation()); // force
																		// update
			} else {
				pages = new MultiDisplayFX(this, pageNumber, decode_pdf.getDynamicRenderer(), displayView, comp,
						options, fileAccess);
			...
			final RenderChangeListener customRenderChangeListener = (RenderChangeListener) externalHandlers
		...
		if (lastDisplayView == Display.PAGEFLOW && displayView != Display.PAGEFLOW) {
			 ...
			 * removeAll();
			 * 
			 * //forms needs null layout manager this.setLayout(null);
			 * 
			 * ((JScrollPane)getParent().getParent()).setVerticalScrollBarPolicy
	...
	public void setDisplayView(final int displayView, final int orientation) {
		...
		if (pageNumber > 0) {
			if (hasChanged && decode_pdf.isOpen()) {
				try {
	 ...
	 * 
	 * @see
	 * org.jpedal.examples.viewer.gui.swing.GUIFactory#resetComboBoxes(boolean)
	 */
	@Override
	...
	public void scaleAndRotate() {
		...
		if (LogWriter.isRunningFromIDE)
			System.out.println("Memory usage after zoom="
					+ (Runtime.getRuntime().totalMemory() - Runtime.getRuntime().freeMemory()) / 1000);

	}

	public static double getScalingRatio(final double actualW, final double actualH, final double maxW,
			final double maxH) {
	...
	public void snapScalingToDefaults(float newScaling) {
		...
		case Display.FACING:
			...
			if (isRotated) {
				...
				// if first or last page double the width, otherwise add other
				// page width
		...
		if (getSelectedComboIndex(Commands.SCALING) != 0
				&& ((newScaling < window_factor * 1.1 && newScaling > window_factor * 0.91)
						|| ((window_factor > scaling && window_factor < newScaling)
								|| (window_factor < scaling && window_factor > newScaling)))) {
			setSelectedComboIndex(Commands.SCALING, 0);
			scaling = window_factor;
		} else if (y_factor != -1 && getSelectedComboIndex(Commands.SCALING) != 1
				&& ((newScaling < y_factor * 1.1 && newScaling > y_factor * 0.91)
						|| ((y_factor > scaling && y_factor < newScaling)
								|| (y_factor < scaling && y_factor > newScaling)))) {
			setSelectedComboIndex(Commands.SCALING, 1);
			scaling = y_factor;
		} else if (x_factor != -1 && getSelectedComboIndex(Commands.SCALING) != 2
				&& ((newScaling < x_factor * 1.1 && newScaling > x_factor * 0.91)
						|| ((x_factor > scaling && x_factor < newScaling)
								|| (x_factor < scaling && x_factor > newScaling)))) {
			setSelectedComboIndex(Commands.SCALING, 2);
			scaling = x_factor;
		} else {
			setSelectedComboItem(Commands.SCALING,
					String.valueOf((int) decode_pdf.getDPIFactory().removeScaling(newScaling * 100)));
			scaling = newScaling;
		}
	}
	...
	public void setupThumbnailPanel() {
		...
		if (thumbnails.isShownOnscreen()) {
			...
			thumbnails.setupThumbnails(decode_pdf.getPageCount(), null, Messages.getMessage("PdfViewerPageLabel.text"),
					decode_pdf.getPdfPageData());

			// add listener so clicking on button changes to page - has to be in
			// Viewer so it can update it
		...
		if (rootNode != null) {
			...
			final EventHandler<MouseEvent> reselectionListener = new EventHandler<MouseEvent>() {
				...
				public void handle(final MouseEvent event) {
					final String ref = ((JavaFXOutline.OutlineNode) ((Labeled) event.getSource()).getUserData())
							.getObjectRef();
					gotoPageByRef(ref);
				}
			};

			((TreeView<Label>) tree.getTree()).getSelectionModel().selectedItemProperty()
					.addListener(new ChangeListener<TreeItem<Label>>() {
						@Override
						public void changed(final ObservableValue<? extends TreeItem<Label>> ov,
								final TreeItem<Label> t, final TreeItem<Label> t1) {
							if (t1 == null) 
								return;
	...
	private void gotoPageByRef(final String ref) {
		...
		if (Aobj != null) {
			decode_pdf.getFormRenderer().getActionHandler().gotoDest(Aobj, ActionHandler.MOUSECLICKED,
					PdfDictionary.Dest);
		}
	}
	 ...
	 * 
	 * @see
	 * org.jpedal.examples.viewer.gui.swing.GUIFactory#setCoordText(java.lang.
	 * String)
	 */
	@Override
	...
	private void createNavbar() {
		 ...
		 * navigation toolbar for moving between pages
		 */
		fxButtons.addButton(NAVBAR, Messages.getMessage("PdfViewerNavBar.RewindToStart"), "start.gif",
				Commands.FIRSTPAGE, menuItems, this, currentCommandListener, pagesToolBar, navToolBar);

		fxButtons.addButton(NAVBAR, Messages.getMessage("PdfViewerNavBar.Rewind10"), "fback.gif", Commands.FBACKPAGE,
				menuItems, this, currentCommandListener, pagesToolBar, navToolBar);

		fxButtons.addButton(NAVBAR, Messages.getMessage("PdfViewerNavBar.Rewind1"), "back.gif", Commands.BACKPAGE,
				menuItems, this, currentCommandListener, pagesToolBar, navToolBar);
		...
		fxButtons.addButton(NAVBAR, Messages.getMessage("PdfViewerNavBar.Forward1"), "forward.gif",
				Commands.FORWARDPAGE, menuItems, this, currentCommandListener, pagesToolBar, navToolBar);

		fxButtons.addButton(NAVBAR, Messages.getMessage("PdfViewerNavBar.Forward10"), "fforward.gif",
				Commands.FFORWARDPAGE, menuItems, this, currentCommandListener, pagesToolBar, navToolBar);

		fxButtons.addButton(NAVBAR, Messages.getMessage("PdfViewerNavBar.ForwardLast"), "end.gif", Commands.LASTPAGE,
				menuItems, this, currentCommandListener, pagesToolBar, navToolBar);

		// add buttons but not in Content Extractor

		fxButtons.addButton(PAGES, Messages.getMessage("PageLayoutButton.SinglePage"), "single.gif", Commands.SINGLE,
				menuItems, this, currentCommandListener, pagesToolBar, navToolBar);
	 ...
	 * 
	 * @see
	 * org.jpedal.examples.viewer.gui.swing.GUIFactory#showMessageDialog(java.
	 * lang.Object)
	 */
	public int showMessageDialog(final Object message1, final Object[] options, final int selectedChoice) {
		...
		if (showMessage) {
			final FXOptionDialog optionsDialog = new FXOptionDialog(stage, message1.toString(), "Message",
					FXOptionDialog.YES_NO_CANCEL_OPTION, options, options[selectedChoice]);
	 ...
	 * 
	 * @see
	 * org.jpedal.examples.viewer.gui.swing.GUIFactory#showInputDialog(java.lang
	 ...
	 * 
	 * @see
	 * org.jpedal.examples.viewer.gui.swing.GUIFactory#showConfirmDialog(java.
	...
	public int showConfirmDialog(final String message, final String message2, final int option) {
		...
		// check user has not setup message and if we still show message
		if (customMessageHandler != null) {
			returnMessage = customMessageHandler
					.requestConfirm(new Object[] { message, message2, String.valueOf(option) });
		}
	 ...
	 * (non-Javadoc)
	 * 
	 * @see org.jpedal.examples.viewer.gui.swing.GUIFactory#showOverwriteDialog(
	 * String file,boolean yesToAllPresent)
	 */
	@Override
	public int showOverwriteDialog(final String file, final boolean yesToAllPresent) {
		...
		if (yesToAllPresent) {
			...
			final Object[] buttonRowObjects = { Messages.getMessage("PdfViewerConfirmButton.Yes"),
			...
			final FXOptionDialog dialog = new FXOptionDialog(stage, message,
					Messages.getMessage("PdfViewerMessage.Overwrite"), FXOptionDialog.DEFAULT_OPTION,
					FXOptionDialog.QUESTION_MESSAGE, buttonRowObjects, buttonRowObjects[0]);
			...
			final FXOptionDialog dialog = new FXOptionDialog(stage, message,
					Messages.getMessage("PdfViewerMessage.Overwrite"), FXOptionDialog.DEFAULT_OPTION,
					FXOptionDialog.QUESTION_MESSAGE, null, null);
	 ...
	 * 
	 * @see
	 * org.jpedal.examples.viewer.gui.swing.GUIFactory#showMessageDialog(javax.
	 * swing.JTextArea)
	 */
	@Override
	...
	public void showFirstTimePopup() {
		...
		final boolean showMessage = (customMessageHandler != null
				&& customMessageHandler.showMessage("first time popup")) || customMessageHandler == null;
		...
		try {
			...
			final Hyperlink supportImg = new Hyperlink("", new ImageView(
					getClass().getResource("/org/jpedal/examples/viewer/res/supportScreenshot.png").toExternalForm()));
			supportImg.setBorder(Border.EMPTY);

			final Hyperlink supportText = new Hyperlink(Messages.getMessage("PdfViewer.SupportLink.Text1") + ' '
					+ Messages.getMessage("PdfViewer.SupportLink.Text2"));
	 ...
	 * @see
	 * org.jpedal.examples.viewer.gui.swing.GUIFactory#showConfirmDialog(java.
	 * lang.Object, java.lang.String, int, int)
	 */
	@Override
	public int showConfirmDialog(final Object message, final String title, final int optionType,
		...
		// check user has not setup message and if we still show message
		if (customMessageHandler != null) {
			returnMessage = customMessageHandler.requestConfirm(
					new Object[] { message, title, String.valueOf(optionType), String.valueOf(messageType) });
		}

		if (returnMessage == -1) {
			final FXOptionDialog dialog = new FXOptionDialog(stage, message, title, optionType, messageType, null,
					null);
	 ...
	 * 
	 * @see
	 * org.jpedal.examples.viewer.gui.swing.GUIFactory#updateStatusMessage(java.
	 * lang.String)
	 */
	@Override
	 ...
	 * 
	 * @see
	 * org.jpedal.examples.viewer.gui.swing.GUIFactory#resetStatusMessage(java.
	 * lang.String)
	 */
	@Override
	 ...
	 * 
	 * @see
	 * org.jpedal.examples.viewer.gui.swing.GUIFactory#setStatusProgress(int)
	 */
	@Override
	...
	public Object printDialog(final String[] printersList, final String defaultPrinter) {
		...
		if (printPanel == null) {
			printPanel = new PrintPanelFX(printersList, defaultPrinter, getPaperSizes(), defaultDPI,
					commonValues.getCurrentPage(), decode_pdf);
			// System.out.println("New print panel!!!!");
		} else {
			printPanel.resetDefaults(printersList, defaultPrinter, commonValues.getPageCount(),
	...
	public void setQualityBoxVisible(final boolean visible) {
		...
		if (properties.getValue("Imageopdisplay").equalsIgnoreCase("true") && qualityBox != null) {
	...
	public void setResults(final GUISearchList results) {
		...
		if (searchInMenu && this.results.getResultCount() == 0) {
			showMessageDialog(
					Messages.getMessage("PdfViewerFileMenuFind.noResultText") + " \"" + results.getSearchTerm() + '"',
					Messages.getMessage("PdfViewerFileMenuFind.noResultTitle"), FXMessageDialog.INFORMATION_MESSAGE);
		}
	}
	 ...
	 * (non-Javadoc)
	 * 
	 * @see org.jpedal.examples.viewer.gui.swing.GUIFactory#enableSearchItems(
	 * boolean)
	 */
	@Override
	public void enableSearchItems(final boolean enabled) {
		...
		if (searchInMenu) { // Menu Bar search
			...
			fxButtons.getButton(Commands.NEXTRESULT).setEnabled(false);
			fxButtons.getButton(Commands.PREVIOUSRESULT).setEnabled(false);
		} else if (fxButtons.getButton(Commands.FIND) != null) { // External
																	// Window
																	// search
	...
	public void decodePage() {
		...
		if (decode_pdf.getDisplayView() == Display.SINGLE_PAGE) {
			// exitTransition =
			// FXViewerTransitions.exitTransition((PdfDecoderFX) decode_pdf,
			// transitionType, TransitionDirection.NONE);
		}

		if (exitTransition != null) {

			// if exists any objects will be stored in this and drawn at correct
			// time
			...
			final Transition entryTransition = FXViewerTransitions.entryTransition((PdfDecoderFX) decode_pdf,
					transitionType, TransitionDirection.NONE);
	...
	protected void setupCenterPanelBackground() {
		...
		if (!propValue.isEmpty() && propValue.equalsIgnoreCase("true")) {
			...
			if (!propValue.isEmpty()) {
				currentCommands.executeCommand(Commands.SETDISPLAYBACKGROUND,
	...
	protected void setupComboBoxes() {
		...
		final String[] scalingValues = { Messages.getMessage("PdfViewerScaleWindow.text"),
		...
		((JavaFXCombo) qualityBox).setStyle("-fx-background-color: #FFFFFF;"); // sets
																				// the
																				// background
																				// colour
																				// to
		...
		scalingBox.setSelectedIndex(defaultSelection); // set default before we
														// add a listener
		...
		navOptionsPanel.getSelectionModel().selectedItemProperty().addListener(new ChangeListener<Tab>() {
			...
			public void changed(final ObservableValue<? extends Tab> ov, final Tab t, final Tab t1) {
				if (navOptionsPanel.getSelectionModel().getSelectedItem() == search) {
					search.setContent(((BrainSearchWindow) searchFrame).getContentVB());
				}
	...
	class RefreshLayout implements ChangeListener {
		...
		public void changed(ObservableValue ov, Object t, Object t1) {
			tracker.startTimer(decode_pdf.getPages(), decode_pdf.getPageNumber(),
					(FileAccess) decode_pdf.getExternalHandler(Options.FileAccess));
	...
	@Override
	public void triggerPageTurnAnimation(PdfDecoderInt decode_pdf, Values commonValues, int updatedTotal,
			boolean rightTurn) {
