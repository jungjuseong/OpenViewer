package org.jpedal.examples.viewer.commands.brain;
...
public class BrainDocInfo {
    ...
    private static void getPropertiesBox(final String selectedFile, final long fileSize, final int pageCount, final int currentPage, final PdfDecoderInt decode_pdf) {
        ...
        final Tab properties = new Tab("일반정보");
        final Tab fonts = new Tab("폰트");
        final Tab available = new Tab("사용가능한 폰트");
        final Tab aliases = new Tab("대체 폰트");
        ...
        if(!xmlText.isEmpty()){
            final Tab xml = new Tab("XML포맷");
            xml.setContent(getXMLTab(xmlText));
            tabPane.getTabs().add(xml);
        }

        final FXDialog newDialog = new FXDialog(null, Modality.APPLICATION_MODAL, tabPane, 400, 500);
        newDialog.setTitle("문서 정보 - " + path + file);
        newDialog.show();
    }
    ...
    private static ScrollPane getPropTab(final PdfFileInformation currentFileInformation, final String file, final String path, final String user_dir, final long size, final int pageCount, final int currentPage, final PdfDecoderInt decode_pdf) {
        ...
        if (currentFileInformation != null) {
             ...
             */
            final Text generalTitle = new Text(lb + "[파일 정보]");
            ...
            final Text generalContent = new Text(lb + "파일이름: " + file + lb
                    + "드라이브: " + path + lb
                    + "현재 경로: " + user_dir + lb
                    + "파일크기: " + size + " K" + lb
                    + "페이지: " + pageCount + lb
             ...
             */
            final Text propertiesTitle = new Text("[크리에이터 정보]");
            ...
            if (currentPageSize != null) {
                final Text pageCoordsTitle = new Text("[페이지 정보]");
                pageCoordsTitle.setFont(headerFont);

                final Text pageCoordsContent = new Text("Media Box: " + currentPageSize.getMediaValue(currentPage) + lb
                        + "Crop Box: " + currentPageSize.getCropValue(currentPage) + lb
                        + "Rotation: " + currentPageSize.getRotation(currentPage) + lb
                );
    ...
    private static ScrollPane getFontTab(final String xmlTxt) {
         ...
         * List of Fonts.
         */
        StringBuilder xmlText = new StringBuilder( "폰트 대체 모드: ");

        switch (FontMappings.getFontSubstitutionMode()) {
            case (1):
                xmlText.append("파일이름을 사용");
                break;
            case (2):
                xmlText.append("PostScript 이름을 사용");
                break;
            case (3):
                xmlText.append("폰트 패밀리 이름을 사용");
                break;
            case (4):
                xmlText.append("풀 폰트 이름을 사용");
                break;
            default:
                xmlText.append("미지 폰트 대체 모드");
                break;
        }
        xmlText.append(lb);
        ...
        if (!xmlTxt.isEmpty()) {
            ...
            final Text generalTitle = new Text(lb + "[폰트 대체 정보]");
            generalTitle.setFont(headerFont);
            final Text fontContent = new Text(xmlText.toString());
            fontContent.setFont(textFont);

            final Text fontListTitle = new Text("[사용가능한 폰트]");
    ...
    private static ScrollPane getImageTab(final PdfDecoderInt decode_pdf) {
        ...
        if (!xmlTxt.isEmpty()) {
            ...
            final Text imageListTitle = new Text(lb + "사용된 이미지 리스트");
    ...
    private static ScrollPane getXMLTab(final String xmlText){
        ...
        final Text imageListTitle = new Text(lb + "[XML 형식]");
        imageListTitle.setFont(headerFont);
    ...
    private static ScrollPane getFormsTab(final PdfDecoderInt decode_pdf) {
        ...
        if (formRenderer != null) {
            ...
            if (formsOnPage != null) {
                ...
                if (formCount == 0)
                	tabTitle = new Text(lb + "Form 객체가 없습니다." + lb);
                else
                	tabTitle = new Text(lb + "Form 객체가 " + formCount + " 개 있습니다." + lb);
    ...
    private static ScrollPane getAliasesTab() {
         ...
         * Add Tab Title
         */
        final Text aliasesTitle = new Text(lb + "[대체 폰트 리스트]");
    ...
    private static VBox getAvailableTab() {
         ...
         * Setup Title.
         */
        final Text titleText = new Text (lb+"[사용가능한 폰트들]" + lb);
        ...
        final Text filterText = new Text("필터");
        ...
        final RadioButton sortFolder = new RadioButton("폴더로 정렬 ");
        sortFolder.setFont(textFont);

        final RadioButton sortName = new RadioButton("이름으로 정렬 ");
        sortFolder.setFont(textFont);
    ...
    private static void getAvailableFonts(final TextField filterText, final boolean sortFontsByDir){
        TreeItem fontlist = new TreeItem("폰트");
